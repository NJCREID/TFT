...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const login = async (userInput: UserLogin) => {
    try {
      setLoading(true);
      const res = await fetchRequest<StoredUser>({
  ...
  const register = async (userInput: UserInput) => {
    try {
      await fetchRequest({
        endpoint: ENDPOINT.AUTH_REGISTER,
        method: "POST",
        body: userInput,
      });
      if (user) {
  ...
  const updateUser = (user: User) => {
    let localUser: StoredUser = JSON.parse(localStorage.getItem("user") || "");
    localUser.user = user;
  ...
  return (
    <AuthContext.Provider value={{ user, updateUser, loading, err, setErr, login, logout, register }}>
      {children}
